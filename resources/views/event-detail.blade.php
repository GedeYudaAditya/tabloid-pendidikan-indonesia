        ...
        <div class="col-md-9">
            ...
            <div class="border p-3">
                ...
                <p class="text-muted">
                    ...
                    {{ $event->created_at->isoFormat('dddd, D MMMM Y HH:mm') }}
                </p>
                <hr>

                <div class="mt-3">
                    {{-- <img src="{{ asset('img/berita/' . $event->gambar) }}" style="width: 50%; float: left" class="me-3"
                        alt="{{ $event->gambar }}"> --}}
                    @if ($event->gambar != null)
                        {{-- slider --}}
                        @if (is_array(json_decode($event->gambar)))
                            <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner">
                                    @foreach (json_decode($event->gambar) as $item)
                                        <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                                            <img src="{{ asset('img/event/' . $item) }}" class="d-block w-100"
                                                style="height: 400px; object-fit: cover; object-position: center"
                                                alt="{{ asset('img/event/' . $item) }}">
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @else
                            <img src="{{ asset('img/event/' . $event->gambar) }}" style="width: 50%; float: left"
                                class="me-3" alt="{{ $event->gambar }}">
                        @endif
                    @else
