        ...
        <div class="col-md-9">
            ...
            <div class="border p-3">
                ...
                <p class="text-muted">
                    ...
                    {{ $jurnalArtikel->created_at->isoFormat('dddd, D MMMM Y HH:mm') }}
                </p>
                <hr>

                <div class="mt-3">
                    {{-- <img src="{{ asset('img/berita/' . $jurnalArtikel->gambar) }}" style="width: 50%; float: left" class="me-3"
                        alt="{{ $jurnalArtikel->gambar }}"> --}}
                    @if ($jurnalArtikel->gambar != null)
                        {{-- slider --}}
                        @if (is_array(json_decode($jurnalArtikel->gambar)))
                            <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner">
                                    @foreach (json_decode($jurnalArtikel->gambar) as $item)
                        ...
                        @else
                            <img src="{{ asset('img/jurnal-arikel/' . json_decode($jurnalArtikel->gambar)) }}"
                                style="width: 50%; float: left" class="me-3" alt="{{ $jurnalArtikel->gambar }}">
                        @endif
                    @else
                    ...
                    {{-- author --}}
                    <p class="mt-3">
                        <span class="text-muted">Dibuat Oleh :</span> {{ $jurnalArtikel->user->name }}
                        <i class="me-3"></i>
                        {{-- shere icon --}}
                        <button type="button" class="shere-button" data-bs-toggle="modal"
                            url="{{ route('guest.berita.detail', $jurnalArtikel->slug) }}" data-bs-target="#exampleModal">
