...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Manajemen Jurnal| Edit Jurnal</h1>
    ...
    <div class="container mb-3">
        ...
        <form action="{{ route('jurnalis.jurnal.update', $jurnal->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="nama_berita1" class="form-label">Judul Jurnal</label>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="isi" id="isi" rows="3">{{ $jurnal->isi }}</textarea>
            </div>

            {{-- preview attachment --}}
            <div class="mb-3">
                <label for="attach" id="attach" class="form-label">Preview Attachment</label>
                @if ($jurnal->attachment != null)
                    <a id="attach" href="{{ asset('attachment/' . $jurnal->attachment) }}" target="_blank"
                        class="btn btn-primary">Preview
                        Attachment
                    </a>
                @else
                    <p>Belum ada attachment</p>
                @endif
            </div>
